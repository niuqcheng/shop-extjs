<template>
  <el-button :loading="isExporting" :type="type" @click="handleExport">
    <slot>导出 Excel</slot>
  </el-button>
</template>
...
const processData = (data: ExportData[]): ExportData[] => {
  ...
  if (Object.keys(props.headers).length > 0) {
    return processedData.map((item) => Object.fromEntries(Object.entries(item).map(([key, value]) => [props.headers[key] || key, value])))
  }
